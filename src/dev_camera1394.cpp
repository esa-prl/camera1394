...
bool Camera1394::findBayerMethod(const char *method)
{
  ...
  if (0 != strcmp(method, "") && BayerPattern_ != DC1394_COLOR_FILTER_NUM)
  {
...
bool Camera1394::readData(sensor_msgs::msg::Image &image)
{
  if (!camera_)
  {
    RCLCPP_FATAL(private_nh_->get_logger(), "Attempt to read from camera that is not open.");
    abort();
  }
  ...
  if (DoBayerConversion_)
  {
    ...
    size_t frame2_size = (frame->size[0] * frame->size[1] * 3 * sizeof(unsigned char));
